...
summaries for model parameters and performance. There is also `easystats`-family
of packages that provide similar functions, more specifically
[parameters](https://easystats.github.io/parameters/) and
[performance](https://easystats.github.io/performance/). Sometimes although
`broom` packages might not contain a method for a given regression object,
`easystats` packages would and vice versa. The hybrid functions in `broomExtra`
make it easy to retrieve these summaries with the appropriate method and does so
robustly.
